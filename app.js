...
container.innerHTML = books.map(book =>   <div class="book-card">   <img src="${book.image_url}" alt="${book.title}">   <h3>${book.title}</h3>   <p class="author">by ${book.author}</p>   <p class="price">₱${parseFloat(book.price).toFixed(2)}</p>   <button class="btn btn-outline" onclick="viewBookDetails('${book.id}')">View Details</button>   </div>  ).join('');
}
...
container.innerHTML = reviews.map(r =>   <div style="background:white; padding:1rem; margin:0.5rem 0; border-radius:8px;">   <strong>Customer</strong>   <span> - ${'⭐'.repeat(Number(r.rating))}</span>   <p>${r.comment}</p>   </div>  ).join('');
}
